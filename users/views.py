...
class UserSettingsView(LoginRequiredMixin,View):
    ...
    def post(self,request,user_id):
        ...
        if password1 == '' and password2 == '' and username != '':
            ...
            user = UserProfile.objects.get(id=user_id)
            return render(request,"settings.html",{"msg": "个人信息修改成功！","user":user})
        #填写了密码项且2次密码相同：
            ...
            return render(request, "settings.html", {"msg": "修改失败：用户名不能为空！","user":user})
        #2次密码不同：
        else:
            return render(request,"settings.html", {"msg": "修改失败：两次输入密码不一致","user":user})
